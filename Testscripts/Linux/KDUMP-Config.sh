...
Config_Rhel() {
    ...
    if [ "$vm2ipv4" ] && [ "$vm2ipv4" != "" ]; then
        ...
        extra_modules="$extra_modules mlx4_en mlx4_core pps_core ptp devlink"
        modprobe -a pci_hv
        if [ $? -eq 0 ];then
            LogMsg "kernel module pci_hv present"
            extra_modules="$extra_modules pci_hv"
        else
            LogMsg "No module pci_hv"
        fi
        # enable -f
...
Config_Debian() {
    ...
    if [ "$vm2ipv4" ] && [ "$vm2ipv4" != "" ]; then
        ...
        LogMsg "Enabling SSH KDUMP"
        ssh_cmd="sed -i '/^#ssh[[:space:]]/c\ssh root@$sshIP' /etc/default/kdump-tools"
        eval "$ssh_cmd"
        sed -i '/^#sshkey[[:space:]]/c\sshkey /root/.ssh/id_rsa' /etc/default/kdump-tools
        # Include extra modules
        extra_modules="$extra_modules mlx4_en mlx4_core pps_core ptp devlink"
        modprobe -a pci_hv
        if [ $? -eq 0 ];then
    ...
    if [ "$extra_modules" ] && [ "$extra_modules" != "" ]; then
        modules_cmd="sed -i '/^#extra_modules[[:space:]]/c\extra_modules $extra_modules' /etc/default/kdump-tools"
        eval "$modules_cmd"
    fi
}
